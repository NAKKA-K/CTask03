...
{
	static Brute1 *brute;
	if (brute->IsCreated() == false) brute = new Brute1();	//コンストラクタの内部で自動的にflagをtrueに変更
...
bool Brute1::m_createdFlag = false;


Brute1::Brute1() :m_scene(0), m_timer(0), m_alpha(0), bright(255)
{
	m_createdFlag = true;
...
void Brute1::BruteGraph()
{
	DrawGraph(0, 0, m_hackBG, FALSE);
	DrawStringToHandle(100, 45, "Cipher", FontList::m_colorGreen1, FontList::m_font75);
	DrawStringToHandle(550, 45, "BruteForce攻撃Ⅰ", FontList::m_colorGreen1, FontList::m_font100);
